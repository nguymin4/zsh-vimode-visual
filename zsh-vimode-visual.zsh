...
vi-visual-highlight()
{
    ...
    if [[ $CURSOR -gt $MARK ]];then
        ...
        __regstart=$MARK
        __regend=$CURSOR_HL
        region_highlight=("${MARK} ${CURSOR_HL} ${VIMODE_VISUAL_HIGHTLIGHT:-standout}")
    elif [[ $MARK -gt $CURSOR ]];then
        (( MARK_HL = MARK + 1 ))
        __regstart=$CURSOR
        __regend=$MARK_HL
        region_highlight=("${CURSOR} ${MARK_HL} ${VIMODE_VISUAL_HIGHTLIGHT:-standout}")
    elif [[ $MARK -eq $CURSOR ]];then
        __regstart=$CURSOR
        __regend=$MARK
        region_highlight=("${CURSOR} ${MARK} ${VIMODE_VISUAL_HIGHTLIGHT:-standout}")
    fi
}
...
vi-vlines-highlight()
{
    if [[ $__start1 == $__start2 ]] && [[ $__end1 == $__end2 ]]; then
        __regstart=$__start1
        __regend=$__end1
        region_highlight=("${__regstart} ${__regend} ${VIMODE_VISUAL_HIGHTLIGHT:-standout}")
        CURSOR=$__savepos
